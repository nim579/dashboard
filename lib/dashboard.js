...
Dashboard.Widgets = Backbone.Collection.extend({
  initialize: function(models, config1) {
    ...
    this.systemView = new Dashboard.SystemView({
      model: this.client
    });
  ...
  preRender: function() {
    return $('body').html('<div id="system"></div><div id="layout"></div>');
  }
});
...
Dashboard.Client = (function(superClass) {
  ...
  Client.prototype.initialize = function(config1) {
    ...
    return this.on('change:version', this.versionCanged);
  };
  ...
  Client.prototype.versionCanged = function() {
    if (this._previousAttributes.version != null) {
...
Dashboard.View = (function(superClass) {
  ...
  View.prototype.render = function() {
    var $container, $widget, grid, i, j, k, len, len1, len2, model, preset, ref, ref1, results, view, views;
    ...
    results = [];
    for (k = 0, len2 = views.length; k < len2; k++) {
      view = views[k];
      view.trigger('readyForRender');
      results.push(view.readyForRender = true);
    }
    return results;
  };
...
Dashboard.SystemView = (function(superClass) {
  ...
  SystemView.prototype.el = '#system';

  SystemView.prototype.initialize = function() {
    console.log(this.model);
    this.listenTo(this.model, 'disconnected', (function(_this) {
      return function() {
        return _this.render('disconnected');
      };
    })(this));
    this.listenTo(this.model, 'connecting', (function(_this) {
      return function() {
        return _this.render('connecting');
      };
    })(this));
    return this.listenTo(this.model, 'connected', (function(_this) {
      return function() {
        return _this.render('connected');
      };
    })(this));
  };

  SystemView.prototype.render = function(event) {
    var $alert, render;
    ...
    if (event === 'connected') {
      ...
      this._TO = setTimeout((function(_this) {
        return function() {
          return _this.$el.hide(400);
        };
      })(this), 2000);
      render = true;
    }
...
Dashboard.widgets.list = (function(superClass) {
  ...
  list.prototype.initialize = function() {
    return this.view = new Dashboard.widgets.listView({
      model: this,
      id: this.id
    });
  };
...
Dashboard.widgets.listView = (function(superClass) {
  ...
  listView.prototype.className = 'widget list';

  listView.prototype.template = '<% if(typeof label != \'undefined\'){ %>\n    <div class="title"><%= label %></div>\n<% } %>\n<span class="value"></span>\n<% if(typeof last_update != \'undefined\'){ %>\n    <div class="helpline">Last updated: <% print(Dashboard.utils.getTime(last_update)) %></div>\n<% } %>';
...
Dashboard.widgets.piechartView = (function(superClass) {
  ...
  piechartView.prototype.template = '<div class="title"><%= label %></div><span class="value">!</span><% if(typeof last_update != \'undefined\'){ %><div class="helpline">Last updated: <% print(Dashboard.utils.getTime(last_update)) %></div><% } %>';
  ...
  piechartView.prototype.initialize = function() {
    ...
    return this.listenToOnce(this.model, 'add', function() {
      if (this.readyForRender) {
        return this.render();
      } else {
        return this.once('readyForRender', this.render);
      }
    });
  };

  piechartView.prototype.render = function() {
    var $canvas, $legend, all, chartEl, color, ctx, data, dataFull, el, i, j, len, newData, ref, ref1;
    piechartView.__super__.render.call(this);
    dataFull = this.getData();
    data = dataFull.value.slice(0);
    this.$el.find('.value').html('<div class="pie"><canvas class="chart"></canvas></div>');
    this.$el.find('.pie').css({
      width: 200,
      height: 200
    });
    $canvas = this.$el.find('.chart');
    $canvas.css({
      width: 200,
      height: 200
  ...
  piechartView.prototype.updateChart = function() {
    var $legend, all, color, data, dataFull, el, element, i, j, k, len, len1, ref, ref1, seg, segment;
    if (this.chart != null) {
      dataFull = this.getData();
      ...
      for (i = j = 0, len = data.length; j < len; i = ++j) {
        el = data[i];
        segment = _.findWhere(this.chart.segments, {
          label: el.label
        });
        if (segment) {
          segment.value = el.value;
        } else {
          this.chart.addData(_.extend({
            color: this.getColor(this.chart.segments.length)
          }, el));
        }
      }
      ref1 = this.chart.segments;
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        seg = ref1[k];
        element = _.findWhere(data, {
          label: seg.label
        });
        if (!element) {
          seg.value = 0;
  ...
  piechartView.prototype.getColor = function(i) {
    var color, colorGen, index, j, ref;
    colorGen = d3.scale.category20();
    color = '';
    for (index = j = 0, ref = i; 0 <= ref ? j <= ref : j >= ref; index = 0 <= ref ? ++j : --j) {
...
Dashboard.widgets.tableView = (function(superClass) {
  ...
  tableView.prototype.template = '<div class="title"><%= label %></div><span class="value"></span><% if(typeof last_update != \'undefined\'){ %><div class="helpline">Last updated: <% print(Dashboard.utils.getTime(last_update)) %></div><% } %>';
