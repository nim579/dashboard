...
Dashboard.widgets.piechartView = (function(superClass) {
  ...
  piechartView.prototype.render = function() {
    var $legend, all, chartEl, color, ctx, data, dataFull, el, i, j, len, newData, ref;
    ...
    if (dataFull.firstAll && ((ref = newData[0]) != null ? ref.value : void 0)) {
      all = newData[0].value;
      newData[0].value = 0;
    }
  ...
  piechartView.prototype.render2 = function() {
    var arc, arcs, center_label, color, data, dataFull, height, label_group, legend, node, pie, pieData, radius, slicer, width;
    piechartView.__super__.render2.call(this);
    dataFull = this.getData();
    data = dataFull.value;
    ...
    if (data.length > 0) {
      ...
      this.chart = d3.select(node).append("svg:svg").data([pieData]).attr("class", "pie").attr("width", width).attr("height", height).append("svg:g").attr("transform", "translate(" + radius + " , " + radius + ")");
      if (dataFull.firstAll) {
        label_group = this.chart.append("svg:g").attr("dy", ".6em");
      ...
      arcs = this.chart.selectAll("g.slice").data(pie).enter().append("svg:g").attr("class", "slice");
      ...
      this.arcs = arcs;
      legend = d3.select(node).append("svg:svg").attr("class", "legend").attr("height", (data.length + data.length * 0.2) + 'em').append("svg:g").attr("x", 0).attr("y", 0).attr("height", (data.length + data.length * 0.2) + 'em').attr('width', 'auto');
      slicer = dataFull.firstAll ? 1 : 0;
      return legend.selectAll("g").data(data.slice(slicer)).enter().append("g").each(function(d, i) {
        var g;
        g = d3.select(this);
        g.append("rect").attr("x", 0).attr("y", (i + 0.2 * i) + 'em').attr("width", '1em').attr("height", '1em').attr("fill", color(i + slicer));
        return g.append("text").attr("x", '1.3em').attr("y", (i + 0.9 + 0.2 * i) + 'em').attr('fill', '#ffffff').attr('font-size', '1em').attr('line-height', '1em').attr("height", '1em').text(d.label + " " + d.value);
      });
    }
  };
